...
import { validate, IsNotEmpty, IsString, ValidationError } from "class-validator";
...
@injectable()
export class GetUserByIdQueryHandler implements RequestHandler<GetUserByIdQuery, Promise<Result<UserDto>>> {
    private readonly _userQuery: IUserQuery;
    constructor(@inject(TYPES.IUserQuery) userQuery: IUserQuery) {
        this._userQuery = userQuery;
    }

    async Handle(request: GetUserByIdQuery): Promise<Result<UserDto>> {
