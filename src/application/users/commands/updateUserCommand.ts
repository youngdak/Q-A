...
export class UpdateUserCommand implements Request<Promise<Result<string | number>>> {
    @IsNotEmpty()
    @IsString()
    ...
    public static create(id: string, body: { firstName: string, lastName: string, otherName: string }): UpdateUserCommand {
...
@injectable()
export class UpdateUserCommandHandler implements RequestHandler<UpdateUserCommand, Promise<Result<string | number>>> {
    private readonly _userrepository: IUserRepository;
    constructor(@inject(TYPES.IUserRepository) userRepository: IUserRepository) {
        this._userrepository = userRepository;
    }

    public async Handle(request: UpdateUserCommand): Promise<Result<string | number>> {
        ...
        if (!user) {
            return ActionResult.fail(`User with the id: '${request.id}' does not exist!`);
        }

        var updateResult = user.update(request.firstName!, request.lastName!, request.otherName!);
